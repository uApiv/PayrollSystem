...
class UserModifyForm(forms.Form):
    ...
    type = forms.CharField(
        label='类型',
        widget=forms.Select(choices=[('小时工', '小时工'), ('委托员工', '委托员工'),
                                     ('受薪雇员', '受薪雇员'), ('管理员', '管理员')],
                            attrs={'onchange': 'func_change()'})
    )
    mail_addr = forms.CharField(
        label='邮寄地址',
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入邮寄地址'})
    )
    ...
    salary_pre_hour = forms.IntegerField(
        label='时薪',
    ...
    salary = forms.FloatField(
        label='工资',
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入工资'}),
        required=False
    )
    com_rate = forms.CharField(
        label='佣金率',
        widget=forms.Select(choices=[('com10', '10%'), ('com15', '15%'),
                                     ('com25', '25%'), ('com35', '35%')]),
        required=False
    )
    std_tax_d = forms.FloatField(
        label='减税率',
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入减税率'}),
        required=False
    )
    other_d = forms.FloatField(
        label='其他扣除',
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入其他扣除'}),
        required=False
    )
    hour_limit = forms.IntegerField(
        label='工时限制',
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入工时限制'}),
        required=False
    )
    password = forms.CharField(label='密码',
                               widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': '请输入密码'}))
...
class PayMethodForm(forms.Form):
    method = forms.CharField(
        label='支付方式',
        widget=forms.Select(choices=[('直接领取', '直接领取'), ('邮寄', '邮寄'),
                                     ('存入银行', '存入银行')],
                            attrs={'onchange': 'func_change()'})
    )
    mail = forms.CharField(
        label='支付地址',
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入支付地址'}),
        required=False
    )
    bank_name = forms.CharField(
        label='银行名称',
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入银行名称'}),
        required=False
    )
    account = forms.CharField(
        label='账户名称',
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入账户名称'}),
        required=False
    )
...
class UserSelectForm(forms.Form):
    keyword = forms.CharField(
        label='关键字段',
        widget=forms.Select(choices=((0,'员工ID'),(1,'姓名'),(2,'职位'),(3,'性别'),(4,'年龄'),(5,'籍贯'),(6,'民族'),(7,'电话'),
                                     (8,'身份证号'),(9,'入职时间'),(10,'时薪')))
    )
