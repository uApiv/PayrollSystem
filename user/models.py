...
class Profile(models.Model):
    ...
    hour_limit=models.IntegerField(verbose_name="hour limit (some employees may not be able to work overtime)", null=True)
    ...
    mail_pay=models.CharField(max_length=100,verbose_name="the address that the paycheck will be mailed to", null=True)
...
def get_staff_mail_addr(self):
    if Profile.objects.filter(user=self).exists():
        profile = Profile.objects.get(user=self)
        return profile.mail_addr
    else:
        return None
...
User.get_staff_mail_addr=get_staff_mail_addr
User.get_user_profile=get_user_profile